...
//Класс трехмерного вектора - нужен для расчета скоростей точек и более компактных записей
class Vect3d{    
    public:
    ...
    Vect3d operator* (const Vect3d& v){
        return Vect3d(y * v.z - v.y * z, z * v.x - v.z * x, x * v.y - y * v.x);
    }
...
public:
    ...
    CalcNode() : r(Vect3d()), smth(0.0), v(Vect3d())
    {
    }

    // Конструктор с указанием всех параметров
    CalcNode(Vect3d r, double smth, Vect3d v) 
            : r(r), smth(smth), v(v)
    {
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            Vect3d r = Vect3d(nodesCoords[i*3], nodesCoords[i*3 + 1], nodesCoords[i*3 + 2]);
            Vect3d v = Vect3d();

            // Модельная скалярная величина распределена как-то вот так
            double smth = pow(sin(r.x * r.x + r.y * r.y + r.z * r.z), 2);
            nodes[i] = CalcNode(r, smth, v);
        }
    ...
    // Обновляет скорости (вращение с постоянной угловой скоростью вокруг фиксированного центра)
    void refresh_velocities(double tau, Vect3d center, Vect3d omega){
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].v = omega * (nodes[i].r - center);
        }
    }
    ...
    void doTimeStep(double tau, Vect3d center, Vect3d omega) {
        ...
        // Обновление скоростей после каждого шага по времени
        refresh_velocities(tau, center, omega);
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        for(unsigned int i = 0; i < nodes.size(); i++) {
            ...
            dumpPoints->InsertNextPoint(nodes[i].r.x, nodes[i].r.y, nodes[i].r.z);

            // Добавляем значение векторного поля в этой точке
            double _vel[3] = {nodes[i].v.x, nodes[i].v.y, nodes[i].v.z};
        ...
        string fileName = "steps/cup-step-" + std::to_string(snap_number) + ".vtu";
